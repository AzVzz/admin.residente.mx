...
import {
  notasPorSeccionCategoriaGet,
  notasTopPorSeccionCategoriaGet,
} from "../../../api/notasPorSeccionCategoriaGet";
...
const MainSeccionesCategorias = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([notasData, restaurantesData, notasTopData]) => {
        ...
        setRestaurantes(
          Array.isArray(restaurantesData) ? restaurantesData : []
        );
  ...
  return (
    <div className="mb-5 mt-9 max-w-[1080px] mx-auto w-full">
      {" "}
      ...
      <div className="flex flex-col md:flex-row gap-8" ref={notasRef}>
        ...
        <div className="flex-1 min-w-0 md:max-w-[80%]">
          {" "}
          {/* min-w-0 previene desbordamientos */}
          ...
          <div className="relative flex justify-center items-center mb-4 mt-0">
            ...
            <div className="relative z-10 px-4 bg-[#DDDDDE]">
              <span className="text-black text-[30px] leading-[0.90] font-black flex-1 overflow-hidden text-center p-2 my-0 tracking-tight uppercase">
                Noticias {categoria}
              </span>
            </div>
          </div>
            ...
            {id ? (
              detalleCargando ? (
                ...
                <div className="text-red-500 p-4">
                  Error al cargar la nota: {errorDetalle?.message}
                </div>
              ) : (
                <DetallePost
                  post={selectedNota}
                  ...
                  <div className="mb-8">
                    <div className="flex flex-col lg:flex-row gap-6 text-center ">
                      ...
                      <div className="lg:basis-[65%] min-w-0">
                        <div
                          ref={(el) =>
                            (notaRefs.current[notasPagina[0].id] = el)
                          }
                          className="w-full"
                        >
      ...
      <div className="overflow-hidden w-full pt-4">
        <BarraMarquee
          categoria={`Más recomendaciones de restaurantes ${categoria}`}
        />
      </div>
      <div className="col-span-2 w-screen relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] py-5">
        <div className="max-w-[1080px] mx-auto w-full">
          <ImagenesRestaurantesDestacados
            restaurantes={restaurantes.slice(0, 6)}
            small
            cantidad={6}
          />
          <div className="mt-5">
            <ImagenesRestaurantesDestacados
              restaurantes={restaurantes.slice(6, 12)}
              small
              cantidad={6}
            />
          </div>
