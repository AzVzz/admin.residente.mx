  ...
  useEffect(() => {
    ...
    Promise.all([catalogoHeadersGet(), catalogoSeccionesGet()])
      .then(([headers, secciones]) => {
        ...
        const anunciate = headers.find((h) =>
          h.seccion.toLowerCase().includes("anunciate")
        );
        ...
        <div className="max-w-[1080px] mx-auto flex justify-between items-center sm:px-5 px-2 sm:py-0.5 py-0">
            ...
            {menuHeader.map((section, idx) =>
              section.url ? (
                ...
                <div key={idx} className="relative group">
                  <a href="#" className="hover:underline text-black font-roman">
                    {section.seccion}
                  </a>
                  {section.submenu && (
                              ...
                              onClick={() =>
                                navigate(
                                  `/seccion/${section.seccion
                                    .replace(/\s+/g, "")
                                    .toLowerCase()}/categoria/${item.nombre
                                    .replace(/\s+/g, "")
                                    .toLowerCase()}`,
                                  {
                                    state: {
          ...
          <div className="sm:flex gap-1.5 hidden">
            <img
              ...
              className="object-contain h-4 w-12 b2b cursor-pointer"
            />
            <a
