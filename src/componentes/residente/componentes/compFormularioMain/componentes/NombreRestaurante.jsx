    ...
    return (
        <div className="mb-4">
            ...
            <input
                ...
                className={`mt-1 block w-full rounded-lg border px-3 py-2 focus:border-indigo-500 bg-white ${errors.nombre_restaurante ? 'border-red-500' : 'border-gray-300'
                    }`}
            />
            <div className="flex justify-between items-center mt-1">
                <span className="text-xs text-gray-500">{nombreRestaurante.length}/22</span>
                {errors.nombre_restaurante && (
                    <p className="text-sm text-red-600">{errors.nombre_restaurante.message}</p>
                )}
            </div>
