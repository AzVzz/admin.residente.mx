                  ...
                  cupones.map((cupon) => (
                    <tr key={cupon.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        {cupon.fecha_validez || "Sin fecha"}
                      </td>
                      <td className="px-6 py-4 align-top text-[12px]">
                        {(() => {
                          const raw = cupon?.secciones_categorias;

                          // 1) Normaliza a arreglo desde string JSON o ya-arreglo
                          let items = [];
                          try {
                          ...
                          // 2) Agrupa por sección y elimina duplicados de categorías
                          const bySeccion = items.reduce((acc, it) => {
                            const seccion = String(it?.seccion ?? "").trim();
                            const categoria = String(
                              it?.categoria ?? ""
                            ).trim();
                            if (!seccion) return acc;
                            acc[seccion] ??= new Set();
                            if (categoria) acc[seccion].add(categoria);
                            return acc;
                          ...
                          // 4) Render
                          const entries = Object.entries(bySeccion);
                          if (entries.length === 0) return "Sin datos";

                          return (
                            <div className="space-y-2">
                              {entries.map(([seccion, cats]) => (
                                <div key={seccion} className="leading-tight">
                                  <div className="font-semibold">
                                    Sección: {seccion}
                                  </div>
                                  {[...cats].map((c) => (
                                    <div key={c} className="pl-3">
                                      Categoría: {titleCase(c)}
                                    </div>
                                  ))}
                                </div>
                              ))}
