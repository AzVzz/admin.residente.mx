...
const getTipoNotaIcon = (nota) => {
  const iconClass =
    "w-45 h-full rounded-bl-lg flex items-center justify-center text-white font-bold text-lg shadow-lg px-2 py-1";
  ...
  if (
    !tipoNota &&
    nota.secciones_categorias &&
    nota.secciones_categorias.length > 0
  ) {
  ...
  if (tipoNota) {
    ...
    if (tipoNota.includes("food") || tipoNota.includes("drink"))
      tipoNota = "food-drink";
    if (tipoNota.includes("antoj")) tipoNota = "antojos";
    if (tipoNota.includes("gastro") || tipoNota.includes("destino"))
      tipoNota = "gastro-destinos";
    if (tipoNota.includes("barrio") || tipoNota.includes("antiguo"))
      tipoNota = "barrio-antiguo";
    if (tipoNota.includes("mama") || tipoNota.includes("rocco"))
      tipoNota = "mama-de-rocco";
...
const NotaCard = ({ nota, onEliminar, eliminando }) => (
  <Link to={`/dashboard/nota/editar/${nota.id}`}>
    <div
      ...
      <div className="flex flex-col justify-between items-end h-full relative z-10">
        ...
        <div className="w-full">
          ...
          {nota.programar_publicacion &&
            esProgramadaFutura(nota.programar_publicacion) && (
...
const esProgramadaFutura = (fechaStr) => {
  ...
  if (fechaStr.includes("T")) {
    ...
    if (match) {
      ...
      fecha = new Date(
        `${anio}-${mes}-${dia}T${hora.toString().padStart(2, "0")}:${minuto}`
      );
    } else {
      return false;
    }
