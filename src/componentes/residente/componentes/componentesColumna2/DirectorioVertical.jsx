...
const DirectorioVertical = () => {
  ...
  return (
    <div
      ref={menuRef}
      className="bg-transparent text-white flex flex-col gap-1.5 relative min-w-full max-w-[348px] items-center pl-10"
    >
      {/**items-center */}
      <div className="flex justify-center">
        <div
          className="absolute left-0 right-0 top-4 border-t-4 border-transparent opacity-100 z-0"
          aria-hidden="true"
        />
        <img
          src="https://residente.mx/fotos/fotos-estaticas/residente-logos/negros/logo-guia-nl.webp"
          className="w-42 h-auto relative z-10 px-4"
        />
      </div>

      <p className="text-[23px] leading-4.5 text-black">
      ...
      <ol className="flex flex-row mt-1 w-full">
        {data.map((seccion, i) => (
          <li
            key={seccion.seccion}
            className="relative group flex flex-col items-center w-full data-item"
          >
            <button
