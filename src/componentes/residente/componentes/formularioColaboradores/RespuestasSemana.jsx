...
const RespuestasSemana = () => {
  ...
  const [vistaActiva, setVistaActiva] = useState(
    tipoParam === "consejo" ? "consejo" : "colaboracion",
  );
  ...
  useEffect(() => {
    const usuarioStr =
      localStorage.getItem("residente_usuario") ||
  ...
  useEffect(() => {
    if (usuario && isColaborador && consejeros.length > 0 && usuario.id) {
      ...
      const consejero = consejeros.find(
        (c) => c.usuario_id === parseInt(usuario.id),
      );
      if (consejero) {
    ...
    if (editarId && isLogged) {
      const loadColaboracion = async () => {
        try {
          ...
          setRespuestaConsejo(
            data.respuesta_consejo === 1 || data.respuesta_consejo === true,
          );
  ...
  const onSubmit = async (data) => {
    ...
    if (!idConsejero) {
      setMensaje(
        "No se encontró tu usuario en la lista de colaboradores. Contacta a soporte.",
      );
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setMensaje(
        editarId
          ? "Error al actualizar la colaboración."
          : "Error al enviar la colaboración.",
      );
    }
    setLoading(false);
  };
          ...
          {!isLogged || !isColaborador ? (
            <div className="p-6 bg-white border border-red-400 rounded text-center">
              ...
              <div className="text-lg text-red-500">
                Este acceso solo está disponible para colaboradores registrados.
                <br />
              ...
              {vistaActiva === "colaboracion" && (
                <FormProvider {...methods}>
                  ...
                  <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    {mensaje && (
                      <div
                        className={`text-center font-bold p-3 rounded-lg mb-4 ${
              ...
              {vistaActiva === "consejo" && (
                <div className="p-6 bg-gray-50 border border-gray-200 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600 mb-4 text-center">
                    Tu consejo será enviado directamente a nuestro equipo y no
                    se publicará en tu perfil.
                  </p>
                  <form
                    onSubmit={async (e) => {
                      e.preventDefault();
                  ...
                  >
                    ...
                    {mensajeConsejo && (
                      <div
                        className={`text-center font-bold mb-4 ${mensajeConsejo.includes("Error") ? "text-red-600" : "text-green-600"}`}
                      >
                        {mensajeConsejo}
                      </div>
                    )}
