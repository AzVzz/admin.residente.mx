...
const SeccionesPrincipales = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const notasFiltradas = todasLasNotas.filter((nota) => {
          ...
          const subtituloNormalizado = nota.subtitulo
            ? normalizarTexto(nota.subtitulo)
            : "";
          ...
          const totalCoincidencias = Math.max(
            coincidenciasTitulo,
            coincidenciasSubtitulo
          );
          const porcentajeCoincidencia =
            totalCoincidencias / palabrasQuery.length;
  ...
  const colsTemplate = data
    .map(
      (s) =>
        seccionesConfig[s.seccion]?.colWidth || seccionesConfig.default.colWidth
    )
    .join(" ");
  ...
  return (
    <div className="w-screen relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] bg-[#fff300] py-8">
                ...
                <div className="w-full flex items-start gap-4">
                  ...
                  <div className="flex-shrink-0">
                    {option.imagen ? (
                      <img
                        src={
                          option.imagen.startsWith("http")
                            ? option.imagen
                            : `${urlApi}${option.imagen}`
                        }
                        alt={option.titulo}
                      ...
                      {inputValue.length >= 3 ? (
                        ...
                        <>
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800 mr-2">
                            {option.tipoNota
                              ? option.tipoNota.replace("-", " ").toUpperCase()
                              : ""}
                          </span>
      ...
      >
        {data.map((seccion, i) => {
          const cfg =
            seccionesConfig[seccion.seccion] || seccionesConfig.default;
