...
const GestionCodigos = () => {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const response = await fetch(`${urlApi}api/usuarios/admin/codigos/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
        ...
        <div className="max-w-[1080px] mx-auto py-10 px-4">
            ...
            <div className="bg-gray-100 p-6 rounded-lg mb-8 shadow-sm">
                ...
                <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    ...
                    <div>
                        <label className="block text-sm font-bold mb-1">Expiración (Opcional)</label>
                        <input
                            type="date"
                    ...
                    <button type="submit" className="bg-[#fff200] text-black font-bold p-2 rounded hover:bg-[#e6d900]">
                        Generar Código
                    </button>
                </form>
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr key={c.id} className="hover:bg-gray-50">
                                <td className="py-2 px-4 border-b font-mono font-bold">{c.codigo}</td>
                                <td className="py-2 px-4 border-b">
                                    <span className={`px-2 py-1 rounded text-xs ${c.tipo === 'permanente' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
                                        {c.tipo}
                                    </span>
                                </td>
                                <td className="py-2 px-4 border-b">
                                ...
                                <td className="py-2 px-4 border-b text-sm text-gray-600">{c.creador?.nombre_usuario || 'Sistema'}</td>
                                <td className="py-2 px-4 border-b text-sm text-gray-600">
                                    {c.createdAt ? new Date(c.createdAt).toLocaleDateString() : (c.fecha_creacion ? new Date(c.fecha_creacion).toLocaleDateString() : '-')}
                                </td>
                                <td className="py-2 px-4 border-b text-sm text-gray-600">{c.expira_en ? new Date(c.expira_en).toLocaleDateString() : '-'}</td>
                                <td className="py-2 px-4 border-b text-sm text-gray-600">{c.usuario_invitado?.nombre_usuario || '-'}</td>
                                <td className="py-2 px-4 border-b text-sm text-gray-600">{c.fecha_uso ? new Date(c.fecha_uso).toLocaleDateString() : '-'}</td>
                                <td className="py-2 px-4 border-b">
                                    <button
