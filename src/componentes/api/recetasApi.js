...
const getToken = () => {
  return localStorage.getItem("residente_token") || null;
};
...
export async function recetasGetMisRecetas(page = 1, limit = 15, filtros = {}) {
  ...
  if (typeof token !== "string" || token.length < 10) {
    ...
    throw new Error(
      "Token de autenticación inválido. Por favor, cierra sesión e inicia sesión nuevamente.",
    );
  }
  ...
  const response = await fetch(
    `${urlApi}api/recetas/mis-recetas?${params.toString()}`,
    {
      headers: {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      console.error(
        "[recetasApi] No se pudo parsear la respuesta de error:",
        e,
      );
    }
    ...
    if (response.status === 403) {
      const errorMessage =
        errorData.error ||
        errorData.message ||
        errorData.errorDato?.error ||
        "No tienes permiso para ver recetas";
    ...
    throw new Error(
      errorData.error ||
        errorData.message ||
        `Error al obtener las recetas (${response.status})`,
    );
  }
